...
$(document).ready(function () {
    $("#tblRolePermissionsList").DataTable({
        processing: true,
        serverSide: true,
        order: [[0, "ASC"]],
...
//JS Validation
$(function () {
    $("form[name='rolePermissions']").validate({
        rules: {
            ddlRole: "required",
        },
        messages: {
    ...
    $.each(MenuList, function (idx, val) {
        if (this.is_mainmenu == "1") {
            if (this.id == "1") {
                ...
                if ($.trim(val.is_mainmenu) == 0 && $.trim(val.is_module) == 0) {
...
function showDelete(id) {
    confirmDelete(id, "delete/permission/", "tblRolePermissionsList");
}
